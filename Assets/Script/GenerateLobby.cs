...
public class GenerateLobby : MonoBehaviour {
	...
	void Start () {
		...
		for (i = 1; i <= size;i++)
		{
            ...
            tm.text = DataScan.rootModel.albumList[i].name;
            tm.color = Color.black;
            tm.fontSize = 36;
            label.transform.SetParent(curDoor.transform);
            label.transform.localPosition = new Vector3(-5, 25, -4);
            label.transform.localRotation = Quaternion.Euler(new Vector3(0, 0, 0));
		...
		for (i=1;i<=secHalf; i++)
		{
            ...
            var label = new GameObject("Label");
            var tm = label.AddComponent<TextMesh>();
            tm.text = DataScan.rootModel.albumList[i+size].name;
            tm.color = Color.black;
            tm.fontSize = 36;
            label.transform.SetParent(curDoor.transform);
            label.transform.localPosition = new Vector3(-5, 25, -4);
